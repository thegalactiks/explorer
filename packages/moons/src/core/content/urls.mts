...
import type { ContentlayerWebPageDocument, ContentlayerWebsite } from './types/index.mjs'
import { documentByIdentifierAndLanguageSelector, documentsByLanguageSelector, pageDepthSelector } from './selectors.mjs'
...
export const computeDocumentsUrl = (websites: ContentlayerWebsite[]) => async (documents: ContentlayerWebPageDocumentWithRender[]) => {
  const getDocumentByIdentifierAndLanguage = documentByIdentifierAndLanguageSelector(documents)
  ...
  const _computePath = (document: ContentlayerWebPageDocument): string => {
    ...
    const parent = getDocumentByIdentifierAndLanguage(document.isPartOf, document.inLanguage) as ContentlayerDocumentWithPath
